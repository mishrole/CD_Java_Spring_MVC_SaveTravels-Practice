...
const Validate = (inputs) => {
    ...
    inputs.forEach(element => {
        ...
        if (element.type === 'checkbox') {
			...
			if (element.classList.contains('validate-minimum')) {
				const min = Number(element.min);
				
				if (!(Number(element.value) >= min)) {
					ShowInvalidFeedback(element, "Must be greater or equals to " + min);
				} else {
					ShowValidFeedback(element);
				}
			}
            
            if (element.classList.contains('validate-minmax')) {
				const max = Number(element.max);
				const min = Number(element.min);
				
				if (!(Number(element.value) >= min && Number(element.value) <= max)) {
					ShowInvalidFeedback(element, "This field has a number range");
				} else {
                	ShowValidFeedback(element);
            	}
			}
...
const ShowInvalidFeedback = (element, message) => {
